...
public:
    ...
    auto enqueue(Func&& f, Args&&... args) -> std::future<typename std::invoke_result<Func, Args...>::type> {
        ...
        auto prepareAsyncTask = std::make_shared<std::packaged_task<returnType()>>(
            [func = std::forward<Func>(f), args = std::make_tuple(std::forward<Args>(args)...)]() mutable {
                std::apply(func, args);
            });
...
int main() {
    ...
    auto task3 = pool.enqueue([](int a, int b) {
        std::cout << "Task 3 executed..." << a + b << std::endl;
    }, 10, 20);
}
